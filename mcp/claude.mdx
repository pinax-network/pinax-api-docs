---
title: 'Claude for Desktop (Beta)'
sidebarTitle: 'Claude'
---

**Claude for Desktop** (Beta) - Your AI partner on desktop. Fast, focused, and designed for deep work.

> https://claude.ai/download

import Authentication from '/snippets/authentication.mdx';
import NpmInstall from '/snippets/npm-install.mdx';
import Troubleshooting from '/snippets/troubleshooting.mdx';
import McpServers from '/snippets/mcp-servers.mdx';

<img width="904" alt="Image" src="https://github.com/user-attachments/assets/75252b6b-2d0c-4abb-90b8-2fc2fb7d53f0" />

<Authentication />

## Configuration

<NpmInstall />

You will need to either create or edit your `Claude/claude_desktop_config.json` file.

> **Settings** > **Developer** > **Edit Config**

- OSX: `~/Library/Application Support/Claude/claude_desktop_config.json`
- Windows: `%APPDATA%\Claude\claude_desktop_config.json`
- Linux: `.config/Claude/claude_desktop_config.json`

<McpServers/>

---

<Troubleshooting />

> ENOENT

![screenshot_2025-03-14_12-12-30](https://github.com/user-attachments/assets/b8d6c4e8-9af5-4168-9f45-00939386a469)

> Server disconnected

![screenshot_2025-03-14_12-15-19](https://github.com/user-attachments/assets/24981bc1-5976-4bda-8a54-3f6ab53a4a5e)

<Note> You can always have a look at the full logs under `Claude/logs/mcp.log` and `Claude/logs/mcp-server-token-api.log` for more details. </Note>